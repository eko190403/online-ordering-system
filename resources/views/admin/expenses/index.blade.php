    ...
    <div class="col-md-4">
        <div class="card bg-danger text-white border-0 shadow-sm rounded-4">
            <div class="card-body">
                ...
                <h3>Rp {{ number_format($expenses->where('expense_date', '>=', date('Y-m-01'))->sum('amount'), 0, ',', '.') }}</h3>
            </div>
        </div>
    </div>
</div>
                    ...
                    @forelse($expenses as $expense)
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                <form action="{{ route('admin.expenses.destroy', $expense->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus pengeluaran ini?')">
                                    @csrf
                                    @method('DELETE')
